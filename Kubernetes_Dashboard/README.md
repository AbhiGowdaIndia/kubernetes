...
* We can just download these two files and run the bash script with the bellow command
  
  **bash dashboard_setup.sh install** 
  Here **install** is an argument, which is used inside the script.

* Without downloading the file just run the bellow command
...
* Here it will ask for tocken and paste the tocken that we copied. Now we can seee the dashboard.
